...
function brief(task, ws, route, extra) {
  ...
  return [
    ...
    `- Evidence or nothing: every number you report must come from a tool output you saved or quote. If a tool fails (quota, 429, timeout), say it failed and set the ticket to blocked. Never write numbers you did not measure. Do not end your run while a background job you started is still running: wait for it or run it in the foreground.`,
    `- Before changing a live server file, back it up on the server and record the backup path.`,
    `- Never look for credentials: do not search ~/.claude, ~/.gemini, browser data, transcripts or other tools' files for keys or tokens, and never write a key into a file. If you need a key you do not have, set the ticket to blocked and say which one.`,

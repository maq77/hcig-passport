...
const ASSETS = {
  ...
  // 24/7 new site (T-014): Pexels service photos, credited in src/assets/CREDITS.md
  C7NURGENT: 'c7n-urgent.webp',
  C7NINJURY: 'c7n-injury.webp',
  C7NLAB: 'c7n-lab.webp',
  C7NIV: 'c7n-iv.webp',
  C7NSPECIALIST: 'c7n-specialist.webp',
  C7NROOMVISIT: 'c7n-roomvisit.webp',
...
/* ---- the 24/7 Clinic site, new design (T-014) ---------------------------- *

   Generated by scripts/gen-247-new.js into src/247new from their brief, served
   at /247clinic/new. Uses the registered assets under /assets. noindex. */

const C7NEW_SRC = path.join(__dirname, 'src', '247new');
if (fs.existsSync(C7NEW_SRC)) {
  fs.cpSync(C7NEW_SRC, path.join(OUT, '247clinic', 'new'), { recursive: true });
}

/* ---- Elite Medical Concierge, rebuilt ------------------------------------ *

   Generated by scripts/build-medcierge.js into src/medcierge and copied here
   verbatim, served at /medcierge. Every page carries its own noindex. */

const MC_SRC = path.join(__dirname, 'src', 'medcierge');
if (fs.existsSync(MC_SRC)) {

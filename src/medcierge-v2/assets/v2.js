/* Elite Medical Concierge, design 2 (lapis and gold). Small, dependency free. Every feature degrades to a
...
(function () {
  ...
  if (menuBtn && menu) {
    ...
    window.addEventListener('resize', function () { if (window.innerWidth >= 1200 && !menu.hidden) close(false); });
  }
  ...
  if (trust) {
    ...
    if (tog) {
      tog.addEventListener('click', function () {
        ...
        tog.setAttribute('aria-label', tog.getAttribute(paused ? 'data-label-play' : 'data-label-pause') || (paused ? 'Play' : 'Pause'));
  ...
  /* colour theme: the system's choice until the visitor picks one, then remembered */
  var themeBtns = document.querySelectorAll('[data-theme-toggle]');
  if (themeBtns.length) {
    var mqDark = window.matchMedia('(prefers-color-scheme: dark)');
    var currentTheme = function () { return doc.getAttribute('data-theme') || (mqDark.matches ? 'dark' : 'light'); };
    var syncTheme = function () {
      var dark = currentTheme() === 'dark';
      themeBtns.forEach(function (b) { b.setAttribute('aria-pressed', String(dark)); });
    };
